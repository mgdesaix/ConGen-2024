...
Next we need to subset the Beagle file to these individuals. I provide a Bash script that uses `awk` to do this (`cut-beagle-ind.awk` in the scripts directory) - it takes as input:

1.  single column file of individual IDs to be kept
2.  *unzipped* Beagle file

You made need to give yourself executable authorization for the script, which can be run on the Terminal:

```sh
chmod +x ./data/cut-beagle-ind.awk
```

Using the balanced western IDs file we created above, we can then subset the full Beagle file. The code below cuts the first column from the `balanced.western.wgsassign.ids.txt` to input the individuals we want to the script, followed by providing the Beagle file (and de-compressing it)
...
2.  Write a new `*.wgsassign.ids.txt` file for these individuals and use the `cut-beagle-ind.awk` script to create a new Beagle file of your "LOO reference" individuals.
...
The first two files we have already worked with. The new "meta data" file provides sampling site names (column = `Site`) for the individuals.
